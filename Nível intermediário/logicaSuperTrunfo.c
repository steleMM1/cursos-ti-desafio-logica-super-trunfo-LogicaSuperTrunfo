#include <stdio.h>

// Desafio Super Trunfo - Países
// Tema 2 - Comparação das Cartas
// Este código inicial serve como base para o desenvolvimento do sistema de comparação de cartas de cidades. 
// Siga os comentários para implementar cada parte do desafio.

int main() {


    //Variáveis carta N°1:
    char estado1[30];
    char nome1[30];
    unsigned long int populacao1;
    float area1;
    float PIB1;
    int qntpontosturisticos1;
    float densidade1;
    float pibcpt1;
    float poder1;


    //Variáveis carta N°2:
    char estado2[30];
    char nome2[30];
    unsigned long int populacao2;
    float area2;
    float PIB2;
    int qntpontosturisticos2;
    float densidade2;
    float pibcpt2;
    float poder2;


    //Variáveis para os menus
    int opcaomenuprincipal;
    int opcaomenujogo;


  //Menu interativo
  

  printf("Menu principal do jogo SuperTrunfo!\n");
  printf("\n");
  printf("Digite 1 para iniciar o jogo.\n");
  printf("Digite 2 para ver as regras do jogo.\n");
  printf("Digite 3 para sair do jogo\n");
  
  scanf("%d", &opcaomenuprincipal);

  switch (opcaomenuprincipal)
  {

  case 1:



    // Cadastro das Cartas:
    // Sugestão: Utilize a função scanf para capturar as entradas do usuário para cada atributo.
    // Solicite ao usuário que insira as informações de cada cidade, como o código, nome, população, área, etc.


    //Aquisição de dados da carta N°1: 

    printf("Digite os dados da carta 1:");
    printf("\n");
    printf("\n");

    printf("Digite o nome do estado que a cidade se encontra:");
    scanf("%s", estado1);

    printf("Digite o nome da cidade:");
    scanf("%s", nome1);

    printf("Digite a população da cidade:");
    scanf ("%lu", &populacao1);   

    printf("Digite a área da cidade:");
    scanf("%f", &area1);

    printf("Digite o PIB da cidade:");
    scanf("%f", &PIB1);

    printf("Digite a quantidade de pontos turísitcos da cidade:");
    scanf("%i", &qntpontosturisticos1);

    //Cálculo das variáveis da carta N°1:
    densidade1 = (float) populacao1 / area1;
    pibcpt1 = (float) PIB1 / populacao1;
    poder1 = (float) ((populacao1 + area1 + PIB1 + qntpontosturisticos1 + pibcpt1 ) - densidade1);

    printf("\n");
    printf("\n");
    printf("\n");

    //Aquisição de dados da carta N°2: 

    printf("Digite os dados da carta 2:");
    printf("\n");
    printf("\n");


    printf("Digite o nome do estado que a cidade se encontra:");
    scanf("%s", estado2);

    printf("Digite o nome da cidade:");
    scanf("%s", nome2);

    printf("Digite a população da cidade:");
    scanf ("%lu", &populacao2);   

    printf("Digite a área da cidade:");
    scanf("%f", &area2);

    printf("Digite o PIB da cidade:");
    scanf("%f", &PIB2);

    printf("Digite a quantidade de pontos turísitcos da cidade:");
    scanf("%i", &qntpontosturisticos2);

    //Cálculo das variáveis da carta N°2:
    densidade2 = (float) populacao2 / area2;
    pibcpt2 = (float) PIB2 / populacao2;
    poder2 = (float) ((populacao2 + area2 + PIB2 + qntpontosturisticos2 + pibcpt2 ) - densidade2);

    printf("\n");
    printf("\n");
    printf("\n");
    printf("\n");
    printf("\n");


    // Exibição dos Dados das Cartas:
    // Sugestão: Utilize a função printf para exibir as informações das cartas cadastradas de forma clara e organizada.
    // Exiba os valores inseridos para cada atributo da cidade, um por linha.
    printf("Dados inseridos das 2 cartas:");
    printf("\n");
    //Carta N°1:
    printf("Dados da carta 1:");
    printf("\n");
  
    printf("Estado: %s \n", estado1);
    printf("Código: A01\n");
    printf("Nome: %s \n", nome1);
    printf("População: %lu habitantes\n", populacao1);
    printf("Área: %f km²\n", area1);
    printf("PIB: %f bilhões de reais\n", PIB1);
    printf("Quantidade de pontos turísticos: %i \n", qntpontosturisticos1);
    printf("Densidade populacional: %.2f hab/km²\n", densidade1);
    printf("PIB per Capta: %.2f reais\n", pibcpt1);

    printf("\n");

  
    //Carta N°2:
    printf("Dados da carta 2:");
    printf("\n");
  
    printf("Estado: %s \n", estado2);
    printf("Código: B02\n");
    printf("Nome: %s \n", nome2);
    printf("População: %lu habitantes\n", populacao2);
    printf("Área: %.2f km²\n", area2);
    printf("PIB: %.2f bilhões de reais\n", PIB2);
    printf("Quantidade de pontos turísticos: %i \n", qntpontosturisticos2);
    printf("Densidade populacional: %.2f hab/km²\n", densidade2);
    printf("PIB per Capta: %.2f reais\n", pibcpt2);

    printf("\n");
    printf("\n");
    printf("\n");
    
    //Menu dentro do jogo
     printf("Escolha o atributo a ser comparado!\n");
     printf("\n");
     printf("Digite 1 para comparar a população.\n");
     printf("Digite 2 para comparar a área.\n");
     printf("Digite 3 para comparar a PIB.\n");
     printf("Digite 4 para comparar a Número de pontos turísticos.\n");
     printf("Digite 5 para comparar a Desidade demográfica.\n");
     scanf("%i", &opcaomenujogo); 
     
    
    
        switch (opcaomenujogo)
        {

        //Comparação das cartas:
        case 1:
            if (populacao1 > populacao2){
              printf("Em %s: %lu tem mais pessoas que em %s: %lu\n", nome1, populacao1, nome2, populacao2);
              printf("A carta 1 é a carta vencedora!");
              printf("\n");
            
              }else if (populacao1 < populacao2) {
                printf("Em %s: %lu tem mais pessoas que em %s: %lu\n", nome2, populacao2, nome1, populacao1);
                printf("A carta 2 é a carta vencedora!");
                printf("\n");
              
            }else {
              printf("Em %s: %lu tem a mesma quantidade de pessoas que em %s: %lu\n", nome1, populacao1, nome2, populacao2);
              printf("Empate!"); 
              printf("\n");
            
              
            }
            break;
        
        
        case 2:
            if (area1 > area2) {
              printf("A área de %s: %.2f é maior que a de %s: %.2f!\n", nome1, area1, nome2, area2);
              printf("A carta 1 é a carta vencedora!");
              printf("\n");
            
              }else if (area1 < area2) {
                printf("A área de %s: %.2f é maior que a de %s: %.2f!\n", nome2, area2, nome1, area1);
                printf("A carta 2 é a carta vencedora!");
                printf("\n");
              
                      
            }else {
              printf("A área de %s: %.2f é a mesma de %s: %.2f!\n", nome1, area1, nome2, area2);
              printf("Empate!"); 
              printf("\n");
            }
            break;


        case 3:
            if (PIB1 > PIB2) {
              printf("O PIB de %s: %.2f é maior que o de %s: %.2f!\n", nome1, PIB1, nome2, PIB2);
              printf("A carta 1 é a carta vencedora!");
              printf("\n");
            
              }else if (PIB1 < PIB2) {
                printf("O PIB de %s: %.2f é maior que o de %s: %.2f!\n", nome2, PIB2, nome1, PIB1);
                printf("A carta 2 é a carta vencedora!");
                printf("\n");
                

            }else {
              printf("O PIB de %s: %.2f é o mesmo de %s: %.2f!\n", nome1, PIB1, nome2, PIB2); 
              printf("Empate!"); 
              printf("\n");
            
            }
            break;


        case 4:
            if (qntpontosturisticos1 > qntpontosturisticos2) {
              printf("Em %s: %i tem mais pontos turísticos que em %s: %i!\n", nome1, qntpontosturisticos1, nome2, qntpontosturisticos2);
              printf("A carta 1 é a carta vencedora!");
              printf("\n");
            
              }else if (qntpontosturisticos1 < qntpontosturisticos2) {
                printf("Em %s: %i tem mais pontos turísticos que em %s: %i!\n", nome2, qntpontosturisticos2, nome1, qntpontosturisticos1);
                printf("A carta 2 é a carta vencedora!");
                printf("\n");
                
          
            }else {
              printf("Em %s: %i tem a mesma quantidade de pontos turísticos que em %s: %i!\n", nome1, qntpontosturisticos1, nome2, qntpontosturisticos2); 
              printf("Empate!");
              printf("\n");
            }
            break;


        case 5:
            if (densidade1 < densidade2) {
              printf("A densidade demográfica em %s: %.2f é menor que em %s: %.2f!\n", nome1, densidade1, nome2, densidade2);
              printf("A carta 1 é a carta vencedora!");
            
              }else if (densidade1 > densidade2) {
              printf("A densidade em %s: %.2f é menor que em %s: %.2f!\n", nome2, densidade2, nome1, densidade1);
              printf("A carta 2 é a carta vencedora!"); 
              printf("\n");
                
        
            }else {
              printf("A densidade demográfica em %s: %.2f é igual em %s: %.2f!\n", nome1, densidade1, nome2, densidade2);
              printf("Empate!"); 
              printf("\n");
            }
              
            break;


        
        default:
        printf("Opção inválida!\n");
        printf("\n");
            break;
        }
        break;

   
  //Exibição das regras
  case 2:
    printf("As regras são simples!\n");
    printf("Digite as informações solicitadas de cada carta\n");
    printf("Após digitar as informações, escolha o atributo a ser comparado para obter a carta vencedora!");
    printf("Rode o código novamente para voltar para o menu\n"); 
    printf("\n");
    break;

  //Sair do jogo
  case 3:
    printf("Saindo do jogo...");
    printf("\n");
    return 0;

  default:
    printf("Opção inválida!\n");
    printf("\n");
    break;
  }



    return 0;
}
